...
func parseI4x4Mode(br *vp8BoolDecoder, topMode, leftMode uint8) uint8 {
	...
	for node > 0 {
		node = yModesIntra4[2*int(node)+int(br.GetBit(prob[node]))]
	}
	return uint8(-node)
}
...
func parseIntraModeRow(
	...
	for mbX := 0; mbX < macroblockWidth; mbX++ {
		...
		var segment uint8
		if updateSegmentMap {
			if br.GetBit(segmentProbs[0]) == 0 {
				segment = uint8(br.GetBit(segmentProbs[1]))
			} else {
				segment = uint8(br.GetBit(segmentProbs[2])) + 2
			}
		}
		...
		if !isI4x4 {
			...
			for y := 0; y < 4; y++ {
				ymode := leftModes[y]
				for x := 0; x < 4; x++ {
					ymode = parseI4x4Mode(br, top[x], ymode)
					top[x] = ymode
					modes[y*4+x] = ymode
				}
				leftModes[y] = ymode
			}
			header.SubModes = modes
		}
	...
	if br.EOF() {
		return nil, bitstreamErr("cannot parse intra mode row")
	}
	return row, nil
}
...
func parseProbabilityTables(br *vp8BoolDecoder) (probabilityTables, error) {
	...
	for typeIndex := range coeffsUpdateProba {
		for bandIndex := range coeffsUpdateProba[typeIndex] {
			for ctxIndex := range coeffsUpdateProba[typeIndex][bandIndex] {
				for probIndex := range coeffsUpdateProba[typeIndex][bandIndex][ctxIndex] {
					prob := coeffsUpdateProba[typeIndex][bandIndex][ctxIndex][probIndex]
					if br.GetBit(prob) == 1 {
						coefficients[typeIndex][bandIndex][ctxIndex][probIndex] = uint8(br.GetValue(8))
						coefficientUpdates++
					}
				}
			}
	...
	useSkipProbability := br.Get() == 1
	var skipProbability *uint8
